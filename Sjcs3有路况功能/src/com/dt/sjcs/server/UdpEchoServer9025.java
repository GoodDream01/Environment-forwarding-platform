...
public class UdpEchoServer9025 {
    ...
    public UdpEchoServer9025(int port) throws SocketException {
	...
	//        	//response = "01"+str.substring(6, 10)+str.substring(2, 6)+"180001020003"+"A5"+"FE"+"04";
	...
	//        	lmzk(request);
				...
				String datastr = "01"+str.substring(6, 10)+str.substring(2, 6)+"18"+sjcdHex+"02"+tdgsHex+datatime+"060101"+dataml+"03";
				response = Crcjs(datastr).replace(" ", "")+"04";
				lmzk(request);
				...
				response = Crcjs(datastr).replace(" ", "")+"04";
			}else if(str.substring(10, 12).equals("40")) {
    ...
    public static void lmzk(String str) throws ParseException {
     // 提取计算的CRC校验值
        String dataToCheck = str.substring(0, str.length() - 6);
        dataToCheck = Crcjs(dataToCheck).replace(" ", "");
        String crcLastTwoCharacters = dataToCheck.substring(dataToCheck.length() - 4, dataToCheck.length() - 2) + dataToCheck.substring(dataToCheck.length() - 2);

        // 获取报文中的校验位
        String expectedCrc = str.substring(str.length() - 6, str.length() - 2);

        // 比较计算的CRC校验值和报文中的校验位是否一致
        if (crcLastTwoCharacters.equalsIgnoreCase(expectedCrc)) {
        	  	
            System.out.println("数据有效，CRC校验一致");
            LmZtbc(str);
        } else {
            System.out.println("数据无效，CRC校验不一致");
        }
        String response;
	}  
    ...
    public static void main(String[] args) throws Exception {
    	System.out.println("打开一个9025端口服务");
        UdpEchoServer9025 server = new UdpEchoServer9025(9025);
        server.start();
    }
 // 定义一个静态方法Ztbc，输入为一个16进制字符串
    public static void LmZtbc(String str) throws ParseException {
        // 解析发送ID，取子字符串从第7个字符到第10个字符，转换为10进制字符串
    	String sbid = new BigInteger(str.substring(6,10),16).toString(10);
        // 解析通道个数，取子字符串从第19个字符到第20个字符，转换为int值
        int tdgs = new BigInteger(str.substring(18,20),16).intValue();
//        String date = "20" + str.substring(26, 28) + "-" + str.substring(28, 30) + "-" + str.substring(30, 32) + " "
//                + str.substring(32, 34) + ":" + str.substring(34, 36) + ":" + str.substring(36, 38);
        // 初始化各种状态变量
        String snh="",rjbbh="",xtsj="",zdsc="",dqsc="",sbsj="",dy="",kzt="",mnzt="",szzt="",sbwd="",sim="",b1310="",b1550="",r1310="",r1550="",lmtempStr="",dqtempStr="",shxs="",sbzt="",smhd="",xh="",bh="";
        // count用于记录解析的当前位置
        int count = 20;
        // 根据通道个数循环解析每个通道
        for(int i=0;i<tdgs;i++) {
            ...
            // 判断通道类型，并解析相应的数据
            if(str.substring(count,count+2).equals("00")) { // 设备SN号
                // 特殊处理，如果以"FFFFFFFFFF"开头，则标记为"FF"
                if(str.substring(count+6, count+6+sum*2).startsWith("FFFFFFFFFF")) {
                    snh="FF";
                }else {
                    // 否则，转换为字符串
			...
			if(str.substring(count,count+2).equals("08")) {//系统状态
				if(str.substring(count+6, count+8).equals("00")) kzt = "正常";
				if(str.substring(count+6, count+8).equals("01")) kzt = "无卡";
				if(str.substring(count+6, count+8).equals("02")) kzt = "初始化失败";
				if(str.substring(count+6, count+8).equals("03")) kzt = "写失败";
			...
			if(str.substring(count,count+2).equals("10")) {//干标定值1310
				b1310 = ""+Double.parseDouble(new BigInteger(str.substring(count+6, count+10),16).toString(10));
			}
			if(str.substring(count,count+2).equals("11")) {//干标定值1550
				b1550 = ""+Double.parseDouble(new BigInteger(str.substring(count+6, count+10),16).toString(10));
			}
			if(str.substring(count,count+2).equals("12")) {//实际值1310
				r1310 = ""+Double.parseDouble(new BigInteger(str.substring(count+6, count+10),16).toString(10));
			}
			if(str.substring(count,count+2).equals("13")) {//实际值1550
				r1550 = ""+Double.parseDouble(new BigInteger(str.substring(count+6, count+10),16).toString(10));
			}
			if (str.substring(count, count + 2).equals("14")) {
			    int lmtempRaw = new BigInteger(str.substring(count + 6, count + 10), 16).intValue();
			    // 检查并转换负值
			    double lmtemp = lmtempRaw > 32767 ? (lmtempRaw - 65536) / 10.0 : lmtempRaw / 10.0;
			    // 将温度值转换为字符串
			    lmtempStr = String.format("%.1f", lmtemp);			 
			}
			// 大气温度
			if (str.substring(count, count + 2).equals("15")) {
			    int dqtempRaw = new BigInteger(str.substring(count + 6, count + 10), 16).intValue();
			    // 检查并转换负值
			    double dqtemp = dqtempRaw > 32767 ? (dqtempRaw - 65536) / 10.0 : dqtempRaw / 10.0;
			    // 将温度值转换为字符串
			    dqtempStr = String.format("%.1f", dqtemp);  
			}
			if(str.substring(count,count+2).equals("16")) {//湿滑系数
				shxs = ""+Double.parseDouble(new BigInteger(str.substring(count+6, count+10),16).toString(10))/100;
			}
			if (str.substring(count, count + 2).equals("17")) { // 设备状态
				// 缩小 10 倍并去掉小数部分
				double sbztRaw = Double.parseDouble(new BigInteger(str.substring(count + 6, count + 10), 16).toString(10)) / 10;
				sbzt = String.valueOf((int) Math.floor(sbztRaw));
			}
			if (str.substring(count, count + 2).equals("18")) { // 水膜厚度
				double smhdRaw = Double.parseDouble(new BigInteger(str.substring(count + 6, count + 10), 16).toString(10))/100;
				smhd = String.format("%.1f", smhdRaw * 10);
			}
			if (str.substring(count, count + 2).equals("19")) { // 雪厚
				double xhRaw = Double.parseDouble(new BigInteger(str.substring(count + 6, count + 10), 16).toString(10));
				xh = String.format("%.1f", xhRaw * 10);
			}
			if (str.substring(count, count + 2).equals("1A")) { // 冰厚
				double bhRaw = Double.parseDouble(new BigInteger(str.substring(count + 6, count + 10), 16).toString(10));
				bh = String.format("%.1f", bhRaw * 10);
			}

//			if(str.substring(count,count+2).equals("17")) {//设备状态
//				sbzt = ""+Double.parseDouble(new BigInteger(str.substring(count+6, count+10),16).toString(10));
//			}
//			if(str.substring(count,count+2).equals("18")) {//水膜厚度
//				smhd= ""+Double.parseDouble(new BigInteger(str.substring(count+6, count+10),16).toString(10))/100;
//			}
//			if(str.substring(count,count+2).equals("19")) {//雪厚
//				xh = ""+Double.parseDouble(new BigInteger(str.substring(count+6, count+10),16).toString(10))/100;
//			}
//			if(str.substring(count,count+2).equals("1A")) {//冰厚
//				bh = ""+Double.parseDouble(new BigInteger(str.substring(count+6, count+10),16).toString(10))/100;
//			}
            // 解析站点运行总时长、当前运行总时长、最近上报时间、系统电压、SD卡状态、模拟通道状态、数字通道状态、设备温度状态、设备SIM号等信息
            // 每个if条件块中的处理逻辑根据通道的具体类型从报文中解析出相应的数据
            // 注意：这里省略了具体的解析逻辑，仅展示了结构
            // 更新count值，准备解析下一个通道
            count = count + 6 + 2*sum;
        }
        System.out.println("sbid: " + sbid);       //设备id         √    
        System.out.println("snh: " + snh);         //SN号           √ 
        System.out.println("rjbbh: " + rjbbh);     //软件版本号      √ 
        System.out.println("xtsj: " + xtsj);       //系统时间        √ 
        System.out.println("kzt: " + kzt);         //系统状态  
        System.out.println("sim: " + sim);         //sim卡号 
        System.out.println("dy" + dy);             //系统电压
        System.out.println("b1310: " + b1310);
        System.out.println("b1550: " + b1550);
        System.out.println("r1310: " + r1310);
        System.out.println("r1550: " + r1550);
        System.out.println("路面温度: " + lmtempStr);
        System.out.println("大气温度: " + dqtempStr);
        System.out.println("shxs: " + shxs);       //湿滑系数      
        System.out.println("sbzt: " + sbzt);       //设备状态
        System.out.println("smhd: " + smhd);       //水膜厚度
        System.out.println("xh: " + xh);           //雪厚
        System.out.println("bh: " + bh);           //冰厚

        // 数据解析完毕后，将解析得到的数据插入数据库
        String id = ComUtils.getUniqueString(); // 生成唯一ID
        ConnDataBase bean = new ConnDataBase(); // 数据库连接实例
        try {
            // 构建插入SQL语句
        	String sql = "insert into sj_sbzt(id, sbid, snh, rjbbh, xtsj, b1310, b1550, r1310, "
        		    + "r1550, lmtempStr, dqtempStr, dy, kzt, sim, time, shxs, sbzt, smhd, xh, bh) values ("
        		    + "'" + id + "', '" + sbid + "', '" + snh + "', '" + rjbbh + "', '" + xtsj + "', '"
        		    + b1310 + "', '" + b1550 + "', '" + r1310 + "', '" + r1550 + "', '" + lmtempStr + "', '"
        		    + dqtempStr + "', '" + dy + "', '" + kzt + "', '" + sim + "', NOW(), '" + shxs + "', '"
        		    + sbzt + "', '" + smhd + "', '" + xh + "', '" + bh + "')";
        	bean.executeUpdate(sql);
		} catch (Exception e) {
			// TODO Auto-generated catch block
