...
public class DsqController {
	...
	public void bzxsj() throws Exception {
		...
		for(int i=0;i<sbgl.size();i++) {
			...
			if(cgqsj.isEmpty()) {
				...
				if(sbgl.get(i).getSffs().equals("0")) {//未发送短信又出现传感器未在线
	...
	public void ycsj() throws Exception {
		...
		for(int i=0;i<sbgl.size();i++) {
			if(sbgl.get(i).getSffs1().equals("0")) {//未发送异常短信的情况下查询连续两次到达6级风速
				...
				if(!cgqsj.isEmpty() && cgqsj.size()>=8) {
					String fsxx = sdf.format(cgqsj.get(1).getTime())+"当前风速为"+cgqsj.get(1).getDqz()+"m/s,风力等级"+getFLDJ(cgqsj.get(1).getDqz())+"级，请注意监测。";
					Cgqyc cgqyc = new Cgqyc();
					cgqyc.setSbbh(sbgl.get(i).getSbbh());
					cgqyc.setState("1");
					cgqyc.setCount(1);
	...
	public static void main(String args[]) throws Exception {
		...
		conn.setAllowUserInteraction(false);		//allowUserInteraction如果为true，则在允许用户交互（例如弹出一个验证对话框）的上下文中对此URL进行检查。		
		PrintStream ps = new PrintStream(conn.getOutputStream());
		String param = "action=send&account=935001&password=cDKy8E&mobile=18231894979&content=【传感器设备】测试内容&extno=106903&rt=json";
		ps.print(param);				
		BufferedReader bReader = new BufferedReader(new InputStreamReader(conn.getInputStream()));				
		String line,resultStr="";				
		while(null != (line=bReader.readLine())){
