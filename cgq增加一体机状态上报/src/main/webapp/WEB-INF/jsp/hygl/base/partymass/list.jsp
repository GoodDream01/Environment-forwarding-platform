...
<!-- 	                        <a class="mini-button" iconCls="icon-remove" onclick="delone()" plain="true">删除</a>          -->
	    ...
	    <div class="mini-fit" >
		    ...
		    <div id="datagrid1" class="mini-datagrid"  style="width:100%;height:100%;"  allowResize="true"
		      url="<%=request.getContextPath() %>/base/partymass/getList.do"    idField="ZID" multiSelect="true" 
		    >
...
<!-- 		            <div field="ztype" width="50" headerAlign="center" align="center" allowSort="true" renderer="onLslxRenderer">隶属</div> -->
	...
	function add() {
		var title="党群系统信息维护";
        mini.open({
            url: HOST_PATH + "/base/partymass/addUi.do",
    ...
    function edit() {
        ...
        if (row) {//alert(row.ZID+":"+row.ZFID);
            mini.open({
            	url: HOST_PATH + "/base/partymass/addUi.do",
                title: "党群系统信息修改", width: 400, height: 400,
                loadOnRefresh: true,
                onload: function () {
                    var iframe = this.getIFrameEl();
    ...
    function view() {
        ...
        if (row) {//alert(row.ZID+":"+row.ZFID);
            mini.open({
            	url: HOST_PATH + "/base/partymass/addUi.do",
                title: "党群系统信息查看", width: 400, height: 400,
                loadOnRefresh: true,
                onload: function () {
                    var iframe = this.getIFrameEl();
    ...
    function deloneDo(){
        ...
        if (row) {//alert(row.ZID+":"+row.ZFID);
            ...
            $.ajax({
                url: HOST_URL+"/base/partymass/delete.do?id="+row.zid,
    			type: 'post',
    		    dataType: 'json',
