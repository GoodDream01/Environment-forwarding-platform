...
<%String zfzid = request.getAttribute("zfzid").toString(); %>
...
<!-- 				<input id="zfzid" name="zfzid" class="mini-textbox" visible="false"/> -->
...
<!-- 	                        <a class="mini-button" iconCls="icon-remove" onclick="delone()" plain="true">删除</a>          -->
	    ...
	    <div class="mini-fit" >
		    ...
		    <div id="datagrid1" class="mini-datagrid"  style="width:100%;height:100%;"  allowResize="true"
		      ...
		      url="<%=request.getContextPath() %>/hygl/wdfz/wj/getList.do?zfzid=<%=zfzid %>" idField="id" multiSelect="true" 
		    ...
		    >
		        <div property="columns">
		            ...
		            <div field="zwjmc" width="360" headerAlign="center" align="center" allowSort="true" >文件名称</div>        
		            <div field="zlrryxm" width="800" headerAlign="center" align="center" allowSort="true" >录入人员</div> 
		            <div field="" width="160" headerAlign="center" align="center" allowSort="true" renderer="onCzRenderer">操作</div>
		            <div field="" width="50" headerAlign="center" align="center" allowSort="false"  renderer="onDownloadRenderer">下载</div>
		        </div>
		    </div>
	    </div>
    </div>
    ...
    function wjlist(){
    	var title="文件管理列表";
        mini.open({
            url: HOST_PATH + "/hygl/wdfz/wjListUi.do",
    ...
    function search() {
        ...
        var zfzid = mini.getByName("zfzid").getValue();
        o.zhyid = zhyid;
        o.zfzid = zfzid;
        grid.load(o);
    }
	...
	function add() {
		var title="文件添加";
        mini.open({
            url: HOST_PATH + "/hygl/wdfz/wj/addUi.do",
    ...
    function edit() {
        ...
        if (row) {//alert(row.ZID+":"+row.ZFID);
            mini.open({
            	url: HOST_PATH + "/hygl/wdfz/wj/addUi.do",
                title: "系统公告修改", width: 600, height: 300,
                loadOnRefresh: true,
                onload: function () {
                    var iframe = this.getIFrameEl();
    ...
    function view() {
        ...
        if (row) {//alert(row.ZID+":"+row.ZFID);
            mini.open({
            	url: HOST_PATH + "/hygl/wdfz/addUi.do",
...
//             return;
            $.ajax({
                url: HOST_URL+"/hygl/dwfz/wj/delete.do?id="+row.zid,
    			type: 'post',
    		    dataType: 'json',
	...
	function getParam(data){
		var o = {};
		o.zfzid = data.id;
		mini.getByName("zfzid").setValue(data.id);
		grid.load(o);
	}
        function expExcel(){
        	var flac = "字典信息";
