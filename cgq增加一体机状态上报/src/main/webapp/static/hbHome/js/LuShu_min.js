var BMapLib = window.BMapLib = BMapLib || {}; (function() {
     ...
     * @return 无返回值.
     */
    c.prototype.goPath = function(points) {
        if (!points || points.length < 1) {
            return;
        }

        var newRoadPoints = points;
        newRoadPoints.unshift(this._path[this._path.length-1]);
        var newRoad = new BMap.Polyline(newRoadPoints, {strokeColor:"red", strokeWeight:2, strokeOpacity:1});
        for(var pl=0;pl<points.length;pl++){
            this._path.push(points[pl]);
        }
        map.addOverlay(newRoad);
        if(!this._moving){
            this.i++;
     ...
     * 获取小车是否正在运行
     */
    c.prototype.getMoving = function() {
        return this._moving;
    }

    c.prototype.changeSpeed = function(speed) {
    ...
    a.object.extend(c.prototype, {
        _addMarker: function(f) {
            ...
            if(isStrNotNull(this._opts.clickFunName)&&typeof(this._opts.clickFunName) == "function"){
                addGraphicEventListener(e,"click",this._opts.clickFunName);
            }
            ...
            i._intervalFlag = setInterval(function() {
                    if (h >= g) {
                        ...
                        if (h == 1) {
                            ...
                            if (i._opts.autoView) {
                                // TODO 这里地图实现自动适应屏幕有bug存在，后续处理
                               // i._map.setViewport([p,i._path[i.i-1]]);
