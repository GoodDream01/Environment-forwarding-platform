...
@Controller
@RequestMapping("/base/party")
public class ParytController {

	private static final Logger logger = Logger.getLogger(ParytController.class);
...
//			User currentUser = UserUtils.getCurrentUser();
	...
	public String addUi(){
		return "/hygl/base/party/add";
	}
	
	@RequestMapping("/save")
	@ResponseBody
	public AjaxJson save(@RequestBody Party party){
		logger.info("保存党派信息....");
		boolean success = false;
		try {
			success = iPartyService.save(party);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("保存党派失败....");
			logger.error(e.getMessage());
	...
	public Party getById(String id){
		logger.info("查询党派信息....id="+id+"");
		Party party = null;
		try {
			party = iPartyService.selectByPrimaryKey(id);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("查询党派信息失败....id="+id+"");
			logger.error(e.getMessage());
		}
		return party;
	...
	public AjaxJson delete(String id){
		logger.info("删除党派信息....id="+id+"");
		boolean success = false;
		try {
			success = iPartyService.delete(id);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("删除党派失败....id="+id+"");
			logger.error(e.getMessage());
		}
		String msg = "删除失败！";
		if(success){
