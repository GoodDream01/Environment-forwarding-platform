...
@Repository
public class RddbxxDaoImpl extends CommonDaoImpl implements RddbxxDaoI{
	...
	public void insert(HashMap hashMap, Connection conn) throws Exception {
		...
		String ZCODE = Tools.convNull((String)hashMap.get("ZCODE"));//编码
		...
		String ZCPPOST = Tools.convNull((String)hashMap.get("ZCPPOST"));
		String ZBIRTHDAY = Tools.convNull((String)hashMap.get("ZBIRTHDAY"));//出生日期
		String ZPOST = Tools.convNull((String)hashMap.get("ZPOST"));//现任职位
		...
		String ZEMAIL = Tools.convNull((String)hashMap.get("ZEMAIL"));//邮箱
		String ZPARTIAN = Tools.convNull((String)hashMap.get("ZPARTIAN"));//党派
		String ZSECTOR = Tools.convNull((String)hashMap.get("ZSECTOR"));//届别
		String ZEDU = Tools.convNull((String)hashMap.get("ZEDU"));//学历
		String ZDEGREE = Tools.convNull((String)hashMap.get("ZDEGREE"));//学位和专业技能职务
		String ZTYPE = Tools.convNull((String)hashMap.get("ZTYPE"));//级别
		String ZNOTE = Tools.convNull((String)hashMap.get("ZNOTE"));//备注
		
		
		String sql= "";
		sql="insert into t_deputies (ZID,ZCODE,ZNAME,ZSEX,ZNATION,ZCPPOST,ZBIRTHDAY,ZPOST,ZADDRESS,ZZIPCODE,ZAREA,ZOFFICETEL,ZMOBILETEL,ZHOUSETEL,ZEMAIL,ZPARTIAN,ZSECTOR,ZEDU,ZDEGREE,ZTYPE,ZNOTE)values(";
		sql+="?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?";
		sql+=")";
	...
	public HashMap getById(String id) {
		// TODO Auto-generated method stub
		String sql = "select * from t_deputies t where t.zid='"+id+"'";
	...
	public void del(String id, Connection conn) throws Exception {
		...
		String sql = "";
		String sqls= "";
		sql="delete from t_deputies where ZID='"+id+"'";
