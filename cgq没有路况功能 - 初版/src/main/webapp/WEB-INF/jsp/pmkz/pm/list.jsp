    ...
    <div title="center" region="center" style="border:0;">
	    ...
	    <div class="mini-fit" >
		    <div id="datagrid1" class="mini-datagrid" style="width:100%;height:100%;" allowResize="true"
		      url="<%=request.getContextPath() %>/pmkz/pm/getList.do" idField="id" multiSelect="true" 
			  fitColumns="false" pageSize="20">
		        <div property="columns">
		            ...
		            <div field="ip" width="200" headerAlign="center" align="center">ip</div>
		            <div field="dk" width="200" headerAlign="center" align="center">端口</div>        
		            <div field="dz" width="200" headerAlign="center" align="center">地址</div> 
		            <div field="bzwz" width="200" headerAlign="center" align="center">布站位置</div>
		            <div field="pmlx" width="200" headerAlign="center" align="center" renderer="onPmlxRenderer">屏幕类型</div>        
...
<script type="text/javascript">
    ...
    var Pmlxs = <%=DictUtils.ItemToKeyValue(DictTypeEnum.PUB_PMKZ_PMLX.toString())%>;
    function onPmlxRenderer(e){
    	return Pmlxs[e.value];
    }
    
	function add() {
		mini.open({
    		url: HOST_PATH + "/pmkz/pm/addUi.do",
    ...
    function kzpm() {    	
        ...
        if (row) {
            mini.open({
            	url: HOST_PATH + "/pmkz/pm/kzUi.do",
                title: "控制屏幕", width: 500, height: 400,
                loadOnRefresh: true,
                onload: function () {
                    var iframe = this.getIFrameEl();
                    var data = { ip:row.ip, dk:row.dk, dz:row.dz };
                    iframe.contentWindow.SetData(data);
                },
                ondestroy: function (action) {
                    grid.reload(); 
    ...
    function yabc() {    	
        ...
        if (row) {
            mini.open({
                ...
                title: "预案保存", width: 500, height: 400,
                loadOnRefresh: true,
                onload: function () {
                    var iframe = this.getIFrameEl();
                    var data = { action:"yabc", id:row.id };
                    iframe.contentWindow.SetData(data);
                },
                ondestroy: function (action) {
                    grid.reload(); 
    ...
    function cxxx() {    	
        ...
        if (row) {
            $.ajax({
                url: HOST_URL+"/pmkz/kz/cxxx.do?id="+row.id,
    			type: 'post',
    		    dataType: 'json',
    ...
    function deloneDo(){
        ...
        if (row) {
            $.ajax({
                url: HOST_URL+"/pmkz/pm/delete.do?id="+row.id,
    			type: 'post',
    		    dataType: 'json',
    		    contentType:'application/json;charset=UTF-8',
                cache: false,
    ...
    function onCzRenderer(e) {
    	var html = "";
    	html += "<a class=\"mini-button\" style=\"width:60px;\" onclick=\"kzpm()\">控制屏幕</a>";
    	html += "<a class=\"mini-button\" style=\"width:60px;\" onclick=\"yabc()\">预案保存</a>";
    	html += "<a class=\"mini-button\" style=\"width:60px;\" onclick=\"cxxx()\">查询信息</a>";
        return html;
    }
