...
<!-- 		           <a class="mini-button" iconCls="icon-excel" onclick="expExcel()">导出Excel</a> -->
	    ...
	    <div class="mini-fit" >
		    ...
		    <div id="datagrid1" class="mini-datagrid"  style="width:100%;height:100%;"  allowResize="true"
		      ...
		      url="<%=request.getContextPath() %>/hygl/fztl/getList.do" idField="id" multiSelect="true" 
		    ...
		    >
		        <div property="columns">
		            ...
		            <div field="zfzmc" width="160" headerAlign="center" align="center" allowSort="true" >分组名称</div>        
		            <div field="ztlyt" width="500" headerAlign="center" align="center" allowSort="true" >讨论议题</div> 
		            <div field="hyinfo.zhymc" width="300" headerAlign="center" align="center" allowSort="true" >会议名称</div> 
		            <div field="" width="430" headerAlign="center" align="center" allowSort="true" renderer="onCzRenderer">操作</div>
		        </div>
		    </div>
	    </div>
    </div>
	...
	function add() {
		var title="分组讨论添加";
        mini.open({
            url: HOST_PATH + "/hygl/fztl/addUi.do",
            title: title, width: 800, height: 660,
            loadOnRefresh: true,
            showMaxButton: true, 
            onload: function () {
    ...
    function edit() {
        ...
        if (row) {//alert(row.ZID+":"+row.ZFID);
            mini.open({
            	url: HOST_PATH + "/hygl/fztl/addUi.do",
                title: "系统公告修改", width: 800, height: 660,
                loadOnRefresh: true,
                onload: function () {
                    var iframe = this.getIFrameEl();
...
//             return;
            $.ajax({
                url: HOST_URL+"/hygl/fztl/delete.do?id="+row.zid,
    			type: 'post',
    		    dataType: 'json',
    ...
    function qrcode() {
        ...
        if (row) {//
            mini.open({
            	url: HOST_PATH + "/hygl/fztl/toQrCodeUi.do?id="+row.zid,
    ...
    function qdcode() {
        ...
        if (row) {//
            mini.open({
            	url: HOST_PATH + "/hygl/fztl/toQdCodeUi.do?id="+row.zid,
            	showHeader:false,	
                title: "二维码", width: 280, height: 310,
                loadOnRefresh: true,
                onload: function () {
    ...
    function fzry() {
        ...
        if (row) {//
            mini.open({
            	url: HOST_PATH + "/hygl/fztl/toFzryUi.do?id="+row.zid,
            	showHeader:false,
                title: "二维码", width: 280, height: 310,
                loadOnRefresh: true,
                onload: function () {
    ...
    function onCzRenderer(e) {
    	...
    	html += "<a class=\"mini-button\" style=\"width:60px;\" onclick=\"qrcode()\">二维码</a>";
    	html += "<a class=\"mini-button\" style=\"width:125px;\" onclick=\"qdcode()\">签到二维码投屏</a>";
    	html += "<a class=\"mini-button\" style=\"width:100px;\" onclick=\"fzry()\">人员投屏</a>";
        return html;
    }
