...
/**
 * @Author lm
 * @Description 百度路线规划obj对象
 * @param speed 轨迹移动速度m/s（默认100000m/s）
 * @param lineColor 轨迹移动线段颜色（ 默认红色）
 * @Date 2018/06/13 19:00:00
 */
var RoadPlanObj = function(){
    this.pointArr = "";
    this.distance = 0;
}
...
function setDefaultValue(trajectoryObj){
    if(isObjectNotNull(trajectoryObj)){
        trajectoryObj.speed = isStrNull(trajectoryObj.speed) ? 100000 : trajectoryObj.speed;
        trajectoryObj.imageUrl = isStrNull(trajectoryObj.imageUrl) ? "etm/images/car.png" : trajectoryObj.imageUrl;
        trajectoryObj.lineColor = isStrNull(trajectoryObj.lineColor) ? "red" : trajectoryObj.lineColor;
        trajectoryObj.lineWeight = isStrNull(trajectoryObj.lineWeight) ? 2 : trajectoryObj.lineWeight;
        trajectoryObj.imageWidth = isStrNull(trajectoryObj.imageWidth) ? 20 : trajectoryObj.imageWidth;
        trajectoryObj.imageHeight = isStrNull(trajectoryObj.imageHeight) ? 20 : trajectoryObj.imageHeight;
        trajectoryObj.title = isStrNull(trajectoryObj.title) ? "石家庄到衡水" : trajectoryObj.title;
    }
