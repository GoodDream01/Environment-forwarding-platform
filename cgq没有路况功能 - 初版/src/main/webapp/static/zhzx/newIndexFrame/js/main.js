	...
	if (document.getElementById("tab" + mkdm)) {
        ...
        if(strlist.indexOf(qxdm)!=-1){
        	...
        	if (isStrNotNull(url) && ("#" != url || ("#" == url && (qxdm=='${pris_enum.MENU_VLM_LIMIT}' || qxdm=='R01')))) {
    			...
    			$('<iframe class="mapFrame" id="frame' + mkdm + '" name="mapFrame" src="'+url+'" allowtransparency="true" marginwidth="0" marginheight="0" hspace="0" vspace="0" frameborder="0" scrolling="auto"></iframe>').appendTo($(".iframeTarget")).siblings(".mapFrame").addClass("downZindex");
    			//getSameOpentabindexTabJquery(obj).trigger("click");
    			//放置tab的ul宽度自增
    			$(".removeAll").css({"right":0}).removeClass("hide");//显示移除全部按钮
