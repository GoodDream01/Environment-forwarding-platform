...
public class UdpEchoServer {
	...
	// 添加 static 关键字，让 channelValues 变成静态变量
	private static String[] channelValues = new String[16];
	// 通道状态映射，去重并定义为全局变量
	private static final Map<String, String> STATUS_MAP = new HashMap<>();
	static {
		STATUS_MAP.put("00", "正常");
		STATUS_MAP.put("01", "极值状态");
		STATUS_MAP.put("02", "异常状态");
		STATUS_MAP.put("03", "通信故障");
		STATUS_MAP.put("FF", "通道无启用");
	}
		...
		if (str.length() > 12) {
			if (str.substring(10, 12).equals("12")) {//定时数据
           	...
           	//01	0030	0010	18	0001	02	00	03	E0AE	04
				...
				response = String.valueOf(Crcjs(datastr).replace(" ", "")) + "04";
				Sjbc(request);
			...
			//服务器回复心跳包：7B 81 00 10 31 33 39 31 32 33 34 35 36 37 38 7B
           	...
           	//01	0001	0010	03	    0006	02	161030094450	03	BBBB	04
				...
				List<Map> list = bean.executeQuery("select * from sj_mlpz where sbid='" + sbid + "' order by xtsj desc");
				String ym = "", dk = "";
				if (list.size() > 0) {
					Map map = list.get(0);
				...
				if (dataml.equals("02") &&
					...
					while (dkHexg.length() < 8)
						dkHexg = "0" + dkHexg;
					String dkHexd = String.valueOf(dkHexg.substring(6, 8)) + dkHexg.substring(4, 6) + dkHexg.substring(2, 4) + dkHexg.substring(0, 2);
					dataml = String.valueOf(dataml) + "0E0304" + dkHexd;
					if (!ym.equals("")) {
						...
						if (ymcdHex.length() == 1)
							ymcdHex = "0" + ymcdHex;
						dataml = String.valueOf(dataml) + "0D05" + ymcdHex + ymHex;
					}
				}
    ...
    public static void Ztbc(String str) throws ParseException {
		String sbid = new BigInteger(str.substring(6,10),16).toString(10);//发送ID
    	int tdgs = new BigInteger(str.substring(18,20),16).intValue() + 1;//通道个数
    	String snh="",rjbbh="",xtsj="",zdsc="",dqsc="",sbsj="",dy="",kzt="",mnzt="",szzt="",sbwd="",sim="",simqd = "";;
    	int count = 20;
		String[] parsedChannelValues = new String[16];
    	for(int i=0;i<tdgs;i++) {
			...
			if (str.substring(count, count + 2).equals("09")) { // 解析一体机状态上报
				String[] channelNames = {
						"temperature", "wind", "pressure", "rainfall", "radiation",
						"ultraviolet", "illumination", "noise", "pm", "humidity",
						"备用11", "备用12", "备用13", "备用14", "备用15", "备用16"
				};
				// 解析并转换通道数据
				for (int j = 0; j < 16; j++) {
					String hexValue = str.substring(count + 6 + j * 2, count + 8 + j * 2);
					channelValues[j] = hexValue; // 16进制数据
					parsedChannelValues[j] = STATUS_MAP.getOrDefault(hexValue, "未知状态(" + hexValue + ")");
				}
			}

			if (str.substring(count, count + 2).equals("0A")) { // 解析数字通道状态
				for (int j = 0; j < sum; j++) {
					String value = str.substring(count + 6 + 2 * j, count + 8 + 2 * j);
					szzt += (j + 1) + " " + STATUS_MAP.getOrDefault(value, "未知状态") + "；";
				}
			}
			...
			if (str.substring(count, count + 2).equals("0F")) { // 解析 SIM 信号强度
				String value = str.substring(count + 6, count + 8); // 获取数据值 (06)
				int simStrength = Integer.parseInt(value, 16); // 16 进制转换为 10 进制
				simqd = String.valueOf(simStrength); // 存入 simqd 变量
				count = count + 8; // 更新解析位置
			}
        ...
        ConnDataBase bean = new ConnDataBase();
		try {
			String sql = "INSERT INTO sj_sbzt2(id, sbid, snh, rjbbh, xtsj, zdsc, dqsc, sbsj, "
					+ "dy, kzt, szzt, sbwd, sim, simqd, time, tahu, wind, pressure, rainfall, radiation, ultraviolet, illumination, noise, pm,"
					+ "备用10, 备用11, 备用12, 备用13, 备用14, 备用15, 备用16) VALUES ("
					+ "'" + id + "', '" + sbid + "', '" + snh + "', '" + rjbbh + "', '" + xtsj + "', '" + zdsc + "', '" + dqsc + "', '" + sbsj + "', "
					+ "'" + dy + "', '" + kzt + "', '" + szzt + "', '" + sbwd + "', '" + sim + "', '" + simqd + "', NOW(), "
					+ "'" + parsedChannelValues[0] + "', '" + parsedChannelValues[1] + "', '" + parsedChannelValues[2] + "', '" + parsedChannelValues[3] + "', "
					+ "'" + parsedChannelValues[4] + "', '" + parsedChannelValues[5] + "', '" + parsedChannelValues[6] + "', '" + parsedChannelValues[7] + "', "
					+ "'" + parsedChannelValues[8] + "', '" + parsedChannelValues[9] + "', '" + parsedChannelValues[10] + "', '" + parsedChannelValues[11] + "', "
					+ "'" + parsedChannelValues[12] + "', '" + parsedChannelValues[13] + "', '" + parsedChannelValues[14] + "', '" + parsedChannelValues[15] + "')";
    ...
    public static void main(String[] args) throws Exception {
    	System.out.println("打开一个9027端口服务");
        UdpEchoServer server = new UdpEchoServer(9027);
        server.start();
    }
